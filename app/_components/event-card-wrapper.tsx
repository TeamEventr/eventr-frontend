...
export default function EventCard({eventDetails, loading} : {eventDetails:EventListHomeResponse, loading?: boolean}) {
    ...
    if (loading){
        return (
            <div className="relative bg-eventr-gray-900 border-2 border-eventr-gray-800 w-52 h-80 flex-shrink-0 flex flex-col rounded-md animate-pulse"/>
        )
    }
    return (
        <div className="relative bg-eventr-gray-900 border-2 border-eventr-gray-800 w-52 flex-shrink-0 flex flex-col rounded-md">
            ...
            <div className="flex gap-2 p-1.5">
                ...
                <div>
                    <h2 className="text-lg truncate w-48">{eventDetails.title}</h2>
                    <p className="text-xs w-40 hidden lg:block truncate-text">{eventDetails.venue}</p>
                </div>
            </div>
        </div>
    )
