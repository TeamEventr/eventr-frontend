...
export default function NavBar() {
    ...
    useEffect(() => {
        if (profileButtonRef.current) {
          console.log('Button ref is working!', profileButtonRef.current);
        }
      }, []);
        ...
        <nav className="flex items-center justify-between px-3 md:px-6 py-2 w-full h-[56px] overflow-x-hidden bg-gradient-to-tr from-zinc-800/30 to-slate-800/30 z-50 border-b-2 border-gray-700/30">
            ...
            <div>
                <button className="active:scale-90 font-bold rounded-full bg-eventr-gray-800 duration-200 hover:bg-eventr-gray-700">
                    <span ref={profileButtonRef} className='flex items-center material-symbols-rounded' style={{ fontSize: '36px', fontVariationSettings: `'FILL' 0, 'wght' 400, 'GRAD' 0, 'opsz' 24`} }>
                        account_circle
                    </span>
                </button>
                <AnimatePresence>
            ...
            <div className="flex gap-2">
                {pathname === '/register' ?
                <Link href='/?login=open' className="py-1.5 active:scale-90 font-bold px-3 rounded-md bg-eventr-gray-800 duration-200 hover:bg-eventr-gray-700">Login</Link>
                :
